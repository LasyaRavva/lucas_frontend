...
export default function DashboardPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto py-10 px-4">
      <h1 className="font-display text-3xl md:text-4xl mb-6">Welcome back{profile?.full_name ? `, ${profile.full_name}` : ''}!</h1>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <Card className="col-span-2">
          ...
          <CardContent>
            <div className="flex items-center gap-6">
              <div>
                <div className="text-4xl font-bold">{profile?.progress ?? 0}%</div>
                <div className="text-sm text-ink/60">Overall Course Completion</div>
              </div>
              <div className="flex flex-col gap-2">
                <Badge variant="coral">Streak: {profile?.streak ?? 0} days</Badge>
                <Badge variant="sea">XP: {profile?.xp ?? 0}</Badge>
              </div>
            </div>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-lg font-semibold mb-2">15 min</div>
