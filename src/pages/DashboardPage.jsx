...
export default function DashboardPage() {
  ...
  if (loading) return <div className="text-center py-10 sm:py-20 text-xs sm:text-sm">Loading dashboard...</div>;
  if (error) return <div className="text-center text-red-500 py-10 sm:py-20 text-xs sm:text-sm">{error}</div>;

  return (
    <div className="w-full min-h-screen">
      <div className="max-w-5xl mx-auto py-6 sm:py-10 px-3 sm:px-4">
        <h1 className="font-display text-2xl sm:text-3xl md:text-4xl mb-4 sm:mb-6 break-words">Welcome back{profile?.full_name ? `, ${profile.full_name}` : ''}!</h1>
